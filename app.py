...
#
# 2026-07-18 修复：
#   新结构记录不再写入 sheet1（sheet1 的表头是旧结构，21列新数据按位置append
#   会跟旧表头错位对不上）。新记录改写入独立分页「比赛记录_v2」，
#   跟"让球盘"分页的做法一致，物理隔离，表头和数据从第一行就对齐。
#   sheet1 保留作历史存档，不再写入，只读。
#
# 2026-07-24 修复：
#   「相似历史比赛参考」从只按WP距离匹配，改成按(主WP,客WP,主EV,客EV)
#   四维归一化欧式距离匹配 —— 单独用WP或单独用EV都容易偏，四个维度一起看
#   才更贴近"这场处境到底像不像"。归一化用候选池的标准差，避免EV(±200+)
#   的数值量级把WP(0-100)的影响盖过去。
# ══════════════════════════════════════════════════════════════════════════════
...
# 用于识别 new_match_id() 生成的比赛ID（迁移工具用）
...
def get_sheet():
    """旧版 sheet1，历史存档，不再写入新结构记录，只保留读取能力（迁移工具用）"""
    return _get_client().open_by_key(SHEET_ID).sheet1


def get_sheet_v2():
    """新结构记录专用分页，做法跟"让球盘"分页一致，不再写入 sheet1"""
...
# ══════════════════════════════════════════════════════════════════════════════
# 相似历史比赛查找
# 2026-07-24：改成(主WP, 客WP, 主EV, 客EV)四维归一化欧式距离匹配
# ══════════════════════════════════════════════════════════════════════════════
...
with tab1:
    ...
    if "e_result" in st.session_state:
        ...
        st.caption("按(主WP,客WP,主EV,客EV)四维距离匹配。默认只在同一「赛事级别」内查找，避免用联赛数据参考国家队比赛")
...
with tab2:
    ...
    if "f_result" in st.session_state:
        ...
        st.divider()
        st.subheader("📊 相似历史比赛参考")
        st.caption("按(主WP,客WP,主EV,客EV)四维距离匹配")
...
with tab5:
    ...
    # ══════════════════════════════════════════════════════════════════════
    # 🔧 一次性迁移工具：把误写进 sheet1 的新结构记录搬到 比赛记录_v2
    # 用完之后请删掉这整个 st.expander 代码块，不要留在生产环境
    # ══════════════════════════════════════════════════════════════════════
    with st.expander("🔧 一次性迁移工具（把sheet1里的新格式记录搬到比赛记录_v2，用完请删除本代码块）"):
        st.caption("识别方式：sheet1里A列是8位hex比赛ID（如 19d67044）的行，就是新算法误写进旧表的记录")
        if st.button("① 扫描 sheet1 中需要迁移的行", key="migrate_scan"):
            sheet1 = get_sheet()
            all_values = sheet1.get_all_values()
            found = []
            for i, row in enumerate(all_values, start=1):
                if row and _ID_PATTERN.match(row[0].strip()):
                    found.append((i, row))
            st.session_state["_migrate_found"] = found
            if found:
                st.write(f"找到 {len(found)} 行：")
                for row_num, raw in found:
                    preview = " | ".join(raw[:6])
                    st.write(f"行{row_num}: {preview}")
            else:
                st.info("没找到需要迁移的行")

        if st.session_state.get("_migrate_found"):
            st.warning("⚠️ 下面这步会把上面列出的行写入「比赛记录_v2」，并从 sheet1 删除，建议先手动备份表格")
            if st.button("② 确认迁移并从sheet1删除", key="migrate_apply", type="primary"):
                found = st.session_state["_migrate_found"]
                v2 = get_sheet_v2()
                sheet1 = get_sheet()
                for row_num, raw in found:
                    values = list(raw) + [""] * (len(HEADERS) - len(raw))
                    v2.append_row(values[:len(HEADERS)])
                for row_num, _r in sorted(found, key=lambda x: x[0], reverse=True):
                    sheet1.delete_rows(row_num)
                st.session_state["_migrate_found"] = []
                load_from_sheet.clear()
                st.success(f"✅ 已迁移 {len(found)} 行到 比赛记录_v2，并从 sheet1 删除。现在可以把这个迁移工具代码块删掉了。")
                st.rerun()
